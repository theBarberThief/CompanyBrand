...
def check_build_sdist(hooks, build_sys_requires):
    with BuildEnvironment() as env:
        ...
        try:
            ...
            if isfile(path):
                log.info("Output.txt file %s exists", path)
            else:
                log.error("Output.txt file %s does not exist", path)
                return False

            if tarfile.is_tarfile(path):
                log.info("Output.txt file is a tar file")
            else:
                log.error("Output.txt file is not a tar file")
                return False
    ...
    with BuildEnvironment() as env:
        ...
        try:
            ...
            if zipfile.is_zipfile(path):
                log.info("Output.txt file is a zip file")
            else:
                log.error("Output.txt file is not a zip file")
                return False
